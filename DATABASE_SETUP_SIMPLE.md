# 🗄️ Настройка базы данных PostgreSQL - Пошаговая инструкция

## 📝 Краткая версия (для опытных)

1. Установите PostgreSQL 14+
2. Создайте базу данных: `CREATE DATABASE marina_crm;`
3. Создайте файл `.env` с параметрами подключения
4. Запустите миграции: `npm run migrate`

---

## 📋 Подробная инструкция по пунктам

### ШАГ 1: Установка PostgreSQL

#### 1.1 Скачивание
- Перейдите на: https://www.postgresql.org/download/windows/
- Нажмите "Download the installer"
- Выберите версию 14 или выше
- Скачайте установщик для Windows (64-bit)

#### 1.2 Установка
1. Запустите установщик
2. Нажимайте "Next" на всех экранах
3. **ВАЖНО:** Запомните пароль для пользователя `postgres`!
4. Порт оставьте по умолчанию: **5432**
5. Дождитесь завершения установки (5-10 минут)

#### 1.3 Проверка
- Откройте **pgAdmin 4** из меню Пуск
- Если открылся — установка успешна!

---

### ШАГ 2: Создание базы данных

#### Способ А: Через pgAdmin (проще)

1. Откройте **pgAdmin 4**
2. Введите пароль (который установили при установке)
3. В левой панели: **Servers** → **PostgreSQL 14** → **Databases**
4. Правой кнопкой на **"Databases"** → **Create** → **Database...**
5. В поле **"Database"** введите: `marina_crm`
6. Нажмите **"Save"**
7. Готово! База данных создана ✅

#### Способ Б: Через командную строку

1. Откройте **PowerShell** или **CMD**
2. Выполните:
   ```bash
   psql -U postgres
   ```
   (Если команда не найдена, используйте полный путь:
   `"C:\Program Files\PostgreSQL\14\bin\psql.exe" -U postgres`)

3. Введите пароль от PostgreSQL
4. Выполните SQL команду:
   ```sql
   CREATE DATABASE marina_crm;
   ```
5. Проверьте:
   ```sql
   \l
   ```
   (должна быть в списке `marina_crm`)
6. Выйдите:
   ```sql
   \q
   ```

---

### ШАГ 3: Создание файла .env

1. В корне проекта создайте файл `.env`
2. Скопируйте туда следующий текст:

```env
# Server Configuration
PORT=3000
NODE_ENV=development

# Database Configuration
DB_HOST=localhost
DB_PORT=5432
DB_NAME=marina_crm
DB_USER=postgres
DB_PASSWORD=ВАШ_ПАРОЛЬ_ОТ_POSTGRES
```

3. **Замените** `ВАШ_ПАРОЛЬ_ОТ_POSTGRES` на пароль, который вы установили при установке PostgreSQL

4. Добавьте также:

```env
# JWT
JWT_SECRET=my-super-secret-jwt-key-change-in-production-12345
JWT_EXPIRES_IN=7d

# File Upload
UPLOAD_MAX_SIZE=10485760
UPLOAD_PATH=./uploads

# Frontend URL
FRONTEND_URL=http://localhost:5173
```

**Пример готового .env файла:**

```env
PORT=3000
NODE_ENV=development

DB_HOST=localhost
DB_PORT=5432
DB_NAME=marina_crm
DB_USER=postgres
DB_PASSWORD=postgres123

JWT_SECRET=my-super-secret-jwt-key-change-in-production-12345
JWT_EXPIRES_IN=7d

UPLOAD_MAX_SIZE=10485760
UPLOAD_PATH=./uploads

FRONTEND_URL=http://localhost:5173
```

---

### ШАГ 4: Проверка подключения

1. Убедитесь, что PostgreSQL запущен:
   - Нажмите `Win + R`
   - Введите `services.msc`
   - Найдите службу **"postgresql-x64-14"**
   - Статус должен быть **"Выполняется"**

2. Запустите миграции:
   ```bash
   npm run migrate
   ```

3. Если всё успешно, увидите:
   ```
   ✅ База данных подключена
   ✅ Схема базы данных синхронизирована
   ```

4. Если ошибка — см. раздел "Решение проблем" ниже

---

### ШАГ 5: Заполнение тестовыми данными (опционально)

```bash
npm run seed
```

Это создаст:
- Тестовых пользователей
- Тестовый яхт-клуб
- Категории расходов

---

## ⚠️ Решение проблем

### Проблема: "password authentication failed"

**Решение:**
- Проверьте пароль в `.env` файле
- Попробуйте подключиться через pgAdmin с тем же паролем

### Проблема: "database does not exist"

**Решение:**
- Создайте базу данных (см. ШАГ 2)
- Проверьте имя в `.env` (должно быть `marina_crm`)

### Проблема: "could not connect" или "connection refused"

**Решение:**
- PostgreSQL не запущен
- Запустите службу PostgreSQL через `services.msc`

### Проблема: "psql is not recognized"

**Решение:**
- Используйте полный путь:
  ```bash
  "C:\Program Files\PostgreSQL\14\bin\psql.exe" -U postgres
  ```

---

## ✅ Чек-лист

Отметьте выполненные пункты:

- [ ] PostgreSQL установлен
- [ ] PostgreSQL запущен (проверено через службы)
- [ ] База данных `marina_crm` создана
- [ ] Файл `.env` создан
- [ ] В `.env` указаны правильные параметры:
  - [ ] DB_HOST=localhost
  - [ ] DB_PORT=5432
  - [ ] DB_NAME=marina_crm
  - [ ] DB_USER=postgres
  - [ ] DB_PASSWORD=ваш_пароль
- [ ] Миграция выполнена успешно (`npm run migrate`)

---

## 🚀 Следующий шаг

После успешной настройки:

```bash
# Заполните тестовыми данными
npm run seed

# Запустите сервер
npm run dev:server
```

---

**Подробное руководство:** См. файл `DATABASE_SETUP.md` для детальных инструкций.

