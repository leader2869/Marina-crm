#!/usr/bin/env bash
# Восстановление и деплой API на VPS, если git pull / npm ci падают.
# Запуск на сервере:
#   bash scripts/vps/repair-and-deploy.sh
# или одной строкой (после появления файла в main):
#   curl -fsSL https://raw.githubusercontent.com/leader2869/Marina-crm/main/scripts/vps/repair-and-deploy.sh | bash
set -euo pipefail
